...
// Seam so a reseal is testable without live KMS/NSM; defaults bind to seal.ts's attestation-gated calls.
...
// the tenantId AAD is preserved, so key identity is unchanged — only the KMS principal that may unseal changes.
